...
At first the model is iterated 10 times, starts with the high value of 63.7 and in the final iteration converges to 46.7. Lower number signifies it is finding the last iteration at which there would be the smallest error possible

```{r logistic_model,warning=FALSE, message=FALSE, echo=FALSE}

set.seed(1234)
dfModel = df_data[sample(nrow(df_data)),]
split = floor(0.6 * nrow(dfModel))
...
n = table(df_data$results_better)
n/sum(n)
cm/colSums(cm)
